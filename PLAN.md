...
- OSC 9 (iTerm2 fallback) and OSC 777 (rxvt) escape sequences raise macOS desktop notifications. libghostty already parses both into a `GHOSTTY_ACTION_DESKTOP_NOTIFICATION` action; a new case in `GhosttyApp.swift`'s `actionCallback` re-broadcasts it as the in-process `.ghosttyDesktopNotification` event
...
- Bell highlight prominence: the orange 6px dot on tabs/sidebar rows was too easy to miss. Replaced with a full-row orange tint (mirroring the active-tab background style) plus an orange leading stripe in the sidebar and an orange-tinted process icon — same visual treatment as the active tab, just in orange. Sites: `TabBarItem.tabBackground`, `SidebarTabRow.rowBackgroundColor` / `leadingStripeColor`
- Session manager renamed-session matching: when a session is renamed via `Cmd+Opt+R` (sets `customName`), the sidebar shows the custom name but the session manager kept showing the path-derived `s.name` and matched fuzzy queries only against that — so typing the new name didn't find the session. `SessionManagerItem.displayName` for `.runningSession` now returns `s.sidebarLabel` (consistent with the sidebar), `subtitle` exposes the original directory path, and `matchableFields` feeds both into FuzzyMatcher (subtitle penalty applies, so a clean displayName hit still wins). Side-benefit: the session manager finally surfaces the directory under each running-session row, useful when two sessions share a custom name
- Copy mode entry — freeze the viewport: the real bug surfaced with `seq 1 1000000000`-style streaming output — entering copy mode kept the viewport in ghostty's `.active` mode (which by definition tracks the bottom of the live screen), so new output kept advancing the visible area and targeted selections were impossible. The viewport has three states (`.top`, `.active`, `.pin` — see `vendor/ghostty/src/terminal/PageList.zig`); only `.pin` stays put while output streams. Fix: new libghostty patch `patches/ghostty/0005-pin-viewport.patch` adds `ghostty_surface_pin_viewport`, which locks the renderer mutex, reads `pages.getTopLeft(.active)` as the current pin row, writes it into `pages.viewport_pin.*`, and flips `pages.viewport` to `.pin` — all with no row movement. `enterCopyMode` calls this single function with no precondition. Earlier iterations went through `scroll_page_lines:-1` (which snaps back to `.active` if the new pin stays inside the active area) and then `mouse_scroll(y = cell_height_px * 2, precision)` (which works but causes a visible ~2-row upward shift on every entry because ghostty's `scrollCallback` thresholds precision deltas at one cell-height and below-threshold values go into `pending_scroll_y` instead of scrolling). Both are scratched. Patch also early-returns on `pages.explicit_max_size == 0` to mirror `PageList.scroll`'s "no-scrollback never leaves `.active`" invariant — keeps the patch a no-op on alternate screens (vim, less, htop, etc.). The secondary "cursor pinned to visible viewport" logic stays in place: when the user enters copy mode while looking at scrollback (`scrollbarState.offset < maxOffset`), the cursor anchors to the middle viewport row (`rows / 2`, col 0) instead of the live cursor row that's offscreen below the pin
